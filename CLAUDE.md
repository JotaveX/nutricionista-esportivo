...
Entry point [backend/src/index.ts](backend/src/index.ts) wires Express, CORS (allow-list driven by `ALLOWED_ORIGIN` env var plus localhost variants), and mounts the click router at `/api`.
...
- `POST /api/click` — resolves the caller's IP via `x-forwarded-for` (Vercel sits behind a proxy, so this header is required; the chain is split to take only the first IP so the value fits the `ip VARCHAR(45)` column), looks up geo via [GeoLocationService](backend/src/geoLocation.ts), and inserts a row.
- `GET /api/clicks` — last 100 rows, pt-BR formatted.
- `GET /api/clicks/stats` — aggregates (totals, top countries/cities, 7-day breakdown).

[backend/src/geoLocation.ts](backend/src/geoLocation.ts) calls the free `ip-api.com` service (HTTP, no key). Localhost IPs (`::1`, `127.0.0.1`) short-circuit to a `Localhost / Desenvolvimento` stub, which the stats query then filters out — keep that filter in sync if the stub strings change.
